...
| 🍊章節名稱  | 🍎內容 |🐇跳轉 |
| ------------- |-------------:|:-------------|
| 資料型別    | 位元組(byte)跟位元(bit)、常見的內建資料型別
資料型別、定義變數、命名限制| [資料型別](#資料型別)    |
| 整數型別簡介    | 位元數字組合、整數的溢位、大數問題  | [整數型別簡介](#整數型別簡介)     |
| 浮點數簡介    | 實數表示法、有效數字、科學記號表示法、科學記號表示法 | [浮點數簡介](#浮點數簡介)     |
| 將printf或scanf用在不同的資料型別(I/O)    | printf的使用、scanf 的使用、隱性轉型與顯性轉型 | [printf或scanf](#將printf或scanf用在不同的資料型別(I/O))     |
